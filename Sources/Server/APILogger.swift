...
/// logged messages.
public enum APILoggerFormatValues: String {
    ...
    static let All: [APILoggerFormatValues] = [
...
public class APILogger {
    ...
    public var format: String? {
        didSet {
            if let format = self.format {
                customFormatter = APILogger.parseFormat(format)
            } else {
                customFormatter = nil
            }
        }
    ...
    public var dateFormat: String? {
        didSet {
            dateFormatter = APILogger.getDateFormatter(format: dateFormat, timeZone: timeZone)
        }
    }
    ...
    fileprivate var dateFormatter: DateFormatter = APILogger.getDateFormatter()
    ...
    private static var tokenRegex: NSRegularExpression? = {
        do {
            ...
            print("Error creating APILogger tokenRegex: \(error)")
            return nil
        }
    }()
    ...
    enum LogSegment: Equatable {
        case token(APILoggerFormatValues)
        case literal(String)
    ...
    static func parseFormat(_ format: String) -> [LogSegment] {
        ...
        for (index, match) in matches.enumerated() {
            ...
            if let formatValue = APILoggerFormatValues(rawValue: segment) {
    ...
    /// Create a `APILogger` instance and set it up as the logger used by the `LoggerAPI`
    /// protocol.
    ...
    public class func use(_ type: LoggerMessageType = .verbose) {
        Log.logger = APILogger(type)
        setbuf(stdout, nil)
    }

    fileprivate let type: LoggerMessageType

    /// Create a `APILogger` instance
    ///
    /// - Parameter type: The most detailed message type (`LoggerMessageType`) to see in the
...
/// Implement the `LoggerAPI` protocol in the `APILogger` class.
