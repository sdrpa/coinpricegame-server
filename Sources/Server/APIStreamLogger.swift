...
/// APILogger, that prints to a TextOutputStream
public class APIStreamLogger<OutputStream: TextOutputStream> : APILogger {

    // stream to output the log to
    private var outputStream: OutputStream

    /// Create a `APIStreamLogger` instance and set it up as the logger used by the `LoggerAPI`
    /// protocol.
    ...
    public static func use(_ type: LoggerMessageType = .verbose, outputStream: OutputStream) {
        Log.logger = APIStreamLogger(type, outputStream: outputStream)
    }
    ...
    override public class func use(_ type: LoggerMessageType = .verbose) {
        print("Unable to instiate APIStreamLogger. " +
              "Use APIStreamLogger.use(:LoggerMessageType:OutputStream) function.")
    }

    /// Create a `APIStreamLogger` instance
    ///
    /// - Parameter type: The most detailed message type (`LoggerMessageType`) to see in the
    ///                  output of the logger.
